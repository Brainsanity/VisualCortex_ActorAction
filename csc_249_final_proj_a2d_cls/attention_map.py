...
def main(args):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(data_loader):
            ...
            output, attentions = model(images).cpu().detach().numpy()
            pdb.set_trace()
            for i in range(attentions.shape[0]):
                f = open( 'attention_maps/Attention_{}_{}_{}_{}_{}.txt'.format( args.data_list, args.net, args.note, batch_idx, i), 'w' )
                for j in range(attentions.shape[1]):
                    for k in range(attentions.shape[2]):
                        f.write('{} '.format(attentions[i,j,k]))
                    f.write('\n')
                f.close()
